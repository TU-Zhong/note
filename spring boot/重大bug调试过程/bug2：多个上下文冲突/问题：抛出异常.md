问题代码：
Ps: 主类不建议直接用getBean()获取，容易引起上下文问题
```java
@Component
@Data
...
最后原因：上下文被多次启动
[[最后的bug原因重复创建了上下文]]
## <mark style="background: #FFF3A3A6;">及其不建议创建多个上下文</mark>
另一个启动类代码：（无报错）
```java
@SpringBootApplication  
...
<mark style="background: #FFF3A3A6;">最后改完能正确读取的代码（手动配置 .yml文件）</mark>
```java
@SpringBootApplication  
//@EnableConfigurationProperties(value.class)  
//@ComponentScan({"com.example.test","com.example.springbootvue3"})  
public class Springbootvue3Application {  
...
```
项目的默认配置类运行没有出现过问题，故数据源配置无问题。因此只能是因为第六点原因的上下文重复配置
比较：
[[两个启动类的代码区别]]
